...
namespace {
    ...
    void identify_client_info(struct sockaddr_in* client_addr_info)
    {
        ...
        if(NULL == inet_ntop(AF_INET, client_addr_info->sin_addr, client_addr_info, INET_ADDRSTRLEN))
        {
...
my_server::my_server(const char* address, uint16_t port)
{
    ...
    m_server_addr_info.sin_addr = INADDR_ANY;
    m_server_addr_info.sin_port = htons(port);

    if_error_exit(
        -1 == bind(m_socket_descriptor, (struct sockaddr *)&m_server_addr_info, sizeof(struct sockaddr_in)),
        "bind");
}
...
void my_server::run()
{
    ...
    while(!m_stopped)
    {
        num_bytes = recvfrom(m_socket_descriptor, buffer, 0, buffer_size_256, (struct sockaddr*)&m_client_addr_info, sizeof(struct sockaddr_in));
        ...
        int num_bytes_sent = sendto(m_socket_descriptor, message, sizeof(message), 0, (struct sockaddr *)&m_client_addr_info, sizeof(struct sockaddr_in));
